...
def backup_to_file(data: Any, name: str, item_type: str) -> str:
    # Create a subdirectory for the item type if it doesn't exist
    type_dir = os.path.join(BACKUP_DIR, item_type.replace(" ", "_").lower())
    os.makedirs(type_dir, exist_ok=True)

    # Save the backup file in the appropriate subdirectory
...
class BackupManager:
    ...
    def _get_workflow_schemes(self) -> List[Dict[str, Any]]:
        """Fetches all workflow schemes and returns the values list."""
        url = f"{self.base_url}/rest/api/3/workflowscheme"
        schemes_data = self._get_json(url)
        return schemes_data.get("values", []) if isinstance(schemes_data, dict) else []
...
# ---------- thread-safe message boxes ----------
    def _show_message(self, level: str, title: str, text: str) -> None:
        """Centralized messagebox + logging. Why: avoid crashes when called from threads."""
...
# ---------- Status helpers ----------
    ...
    def _pump_status_queue(self, event=None):
...
def _merge_unique(self, items: List[Dict[str, Any]], extra: List[Dict[str, Any]], id_key: str, item_type: str) -> List[
    Dict[str, Any]]:
    """
...
# ---------- Fetchers (unused lists) ----------
    def get_unused_custom_fields(self) -> List[Dict[str, Any]]:
        print(f"Current domain: {self.current_domain}")  # Add this line
        url = f"{self.current_domain}/rest/api/3/field"
        fields = self._get(url, "custom fields")
        # Filter out locked fields and fields associated with screens or projects
        return [
            f for f in fields
    ...
    def get_unused_workflows(self) -> List[Dict[str, Any]]:
        """
        Fetches all workflows and returns only those that are not associated
        with any active workflow schemes.
        """
        self.update_status_and_pump("Fetching all workflows...", True)
        url_workflows = f"{self.current_domain}/rest/api/3/workflow"
        all_workflows_data = self._get(url_workflows, "workflows")
        all_workflows = all_workflows_data.get("values", []) if isinstance(all_workflows_data, dict) else []

        self.update_status_and_pump("Fetching all workflow schemes...", True)
        url_schemes = f"{self.current_domain}/rest/api/3/workflowscheme"
        all_schemes_data = self._get(url_schemes, "workflow schemes")
        all_schemes = all_schemes_data.get("values", []) if isinstance(all_schemes_data, dict) else []

        active_workflows: Set[str] = set()
        for scheme in all_schemes:
            for item in scheme.get("issueTypeMappings", {}).values():
                active_workflows.add(item.get("workflow"))

        unused_workflows = [
            wf for wf in all_workflows
            if wf.get("name") not in active_workflows
        ]

        return unused_workflows
    ...
    def get_unused_dashboards(self) -> List[Dict[str, Any]]:
        """
        Fetches all dashboards visible to the user and returns only those not marked as favorite.
        """
        url = f"{self.current_domain}/rest/api/3/dashboard"
        all_dashboards_data = self._get(url, "dashboards")

        # Ensure a valid list of dashboards is returned
        ...
        # Filter the list to include only dashboards where 'favourite' is False
        non_favorite_dashboards = [
            dashboard_data for dashboard_data in all_dashboards if not dashboard_data.get("favourite", False)
        ]

        return non_favorite_dashboards

    def get_unused_filters(self) -> List[Dict[str, Any]]:
        """
        Fetches all filters visible to the user and returns only those not marked as favorite.
        """
        url = f"{self.current_domain}/rest/api/3/filter/search"
        all_filters_data = self._get(url, "filters")

        # Ensure a list of filters was returned
        if not isinstance(all_filters_data, dict) or "values" not in all_filters_data:
            return []

        all_filters = all_filters_data.get("values", [])

        # Filter the list to include only filters where 'favourite' is False
        non_favorite_filters = [
            filter_data for filter_data in all_filters if not filter_data.get("favourite", False)
        ]

        return non_favorite_filters
    ...
    def delete_custom_field(self, field_id: str) -> bool:
        ...
        # Backup the custom field before deletion
        try:
            field_data = self._get(f"{self.current_domain}/rest/api/3/field/{field_id}", item_type="field")
            field_name = field_data.get("name", "unknown_name")
            backup_to_file(field_data,
                           f"custom_field_{field_id}_{field_name}_backup_{datetime.now().strftime('%Y%m%d%H%M%S')}", item_type="field")
        except Exception as e:
