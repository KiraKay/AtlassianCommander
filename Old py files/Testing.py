...
def log_to_file(type_str: str, message: str):
    print(f"[{type_str.upper()}] {message}")
...
class JiraToolGUI(tk.Tk):
    ...
    def _find_fields_in_backups(self, backup_dir: str, field_names: List[str]) -> List[Dict[str, Any]]:
        """
        Scans a backups folder and its subfolders for custom field data within XML backup files.
        """
        found_data = []
        if not os.path.isdir(backup_dir):
            log_to_file("backup_scan", f"Backup directory not found: {backup_dir}")
            return found_data

        # Use os.walk for deep scanning if the variable is set
        if self.recover_deep_scan.get():
            walk_generator = os.walk(backup_dir)
        else:
            # Only walk the top-level directory
            walk_generator = [
                (backup_dir, [], [f for f in os.listdir(backup_dir) if os.path.isfile(os.path.join(backup_dir, f))])]

        for root, _, files in walk_generator:
            for filename in files:
                if filename.endswith(".xml"):
                    file_path = os.path.join(root, filename)
                    log_to_file("backup_scan", f"Scanning backup file: {file_path}")

                    try:
                        tree = ET.parse(file_path)
                        jira = tree.getroot()

                        for issue in jira.iter("issue"):
                            issue_key_elem = issue.find("key")
                            if issue_key_elem is None:
                                continue
                            issue_key = issue_key_elem.text

                            for customfield in issue.iter("customfield"):
                                customfieldname_elem = customfield.find("customfieldname")
                                if customfieldname_elem is not None and customfieldname_elem.text in field_names:
                                    field_name = customfieldname_elem.text
                                    field_id = customfield.get("id")

                                    values = []
                                    for customfieldvalue in customfield.iter("customfieldvalue"):
                                        if customfieldvalue.text:
                                            values.append(customfieldvalue.text)
                                    ...
                                    log_to_file("backup_scan",
                                                f"Found data for '{field_name}' in {issue_key} in backup.")
                    except ET.ParseError as e:
                        log_to_file("error", f"Error parsing XML file {file_path}: {e}")
        return found_data

    def _scan_backups(self):
        backup_path = self.backup_path_entry.get()
        field_names_str = self.field_names_entry.get()
        field_names = [name.strip() for name in field_names_str.split(",") if name.strip()]

        if not backup_path or not field_names:
            self.status_queue.put("Please provide a backup path and at least one field name.")
            return

        self.status_queue.put("Scanning backups... This may take a while.")

        # Run the scan in a separate thread to prevent GUI freezing
        threading.Thread(target=self._run_backup_scan_in_thread, args=(backup_path, field_names)).start()

    def _run_backup_scan_in_thread(self, backup_path, field_names):
        try:
            found_data = self._find_fields_in_backups(backup_path, field_names)
            self.after(0, self._display_backup_results, found_data)  # Update GUI on main thread
        except Exception as e:
            self.status_queue.put(f"Error during backup scan: {e}")

    def _display_backup_results(self, found_data):
        # Clear the custom fields treeview and populate with backup results
        tree = self.recover_trees["Custom Fields"]
        tree.delete(*tree.get_children())

        if not found_data:
            self.status_queue.put("Backup scan complete. No data found for the specified fields.")
        else:
            for item in found_data:
                # Assuming your treeview columns can accommodate this
                tree.insert("", "end", values=(
                    f"{item['field_name']} (from backup)",
                    item["field_id"],
                    item["issue_key"],
                    item["source_file"]
                ))
            self.status_queue.put(f"Backup scan complete. Found {len(found_data)} matching entries.")

    def _browse_backup_path(self):
        folder_selected = filedialog.askdirectory()
        if folder_selected:
            self.backup_path_entry.delete(0, tk.END)
            self.backup_path_entry.insert(0, folder_selected)
    ...
    def _delete(self, url: str) -> bool:
        try:
            ...
            self._show_message("error", "Deletion Failed", f"Failed to delete item: {e}")  # Add this
            return False
    ...
    def delete_custom_field(self, field_id: str) -> bool:
        """
         Backup custom field metadata, contexts, and options before deletion.
         The backup filename uses the field name and type for clarity.
         """
        try:
            # Fetch field details
            field_data = self._get_json(f"/rest/api/3/field/{field_id}") or {}
            field_name = field_data.get("name", "unnamed")
            safe_name = re.sub(r'[^A-Za-z0-9_-]+', '_', field_name).strip('_') or 'unnamed'
            # Fetch context(s)
            contexts = self._get_json(f"/rest/api/3/field/{field_id}/contexts") or {}
            # Fetch options for each context
            options_by_ctx = {}
            for ctx in contexts.get("values", []):
                ctx_id = ctx.get("id")
                if ctx_id:
                    options_by_ctx[ctx_id] = self._get_json(
                        f"/rest/api/3/field/{field_id}/context/{ctx_id}/option") or {}
            # Combine all data
            backup_data = {
                "field": field_data,
                "contexts": contexts,
                "optionsByContext": options_by_ctx
            }
            backup_to_file(backup_data, f"{safe_name}_Custom_Field", "Custom Fields")
            # Proceed with deletion
            url = f"{self.current_domain.rstrip('/')}/rest/api/3/field/{field_id}/delete"
            resp = requests.post(url, auth=self.jira_auth(), timeout=TIMEOUT)
            if resp.status_code == 204:
                log_to_file("cleanup", f"Successfully deleted custom field {field_id}")
                return True
            elif resp.status_code == 400:
                log_to_file("cleanup", f"Cannot delete field {field_id} - it may still be in use")
                return False
            else:
                resp.raise_for_status()
                return True
        except Exception as e:
            log_to_file("cleanup", f"Failed to backup or delete custom field {field_id}: {e}")
            return False
    ...
    def _build_recover_tab(self) -> None:
        ...
        for name in self.recover_types.keys():
            # This frame variable is local to each iteration of the loop
            frame = ttk.Frame(notebook)
            notebook.add(frame, text=name)
            ...
            # --- Include backup scanning for Custom Fields only ---
            ...
            # Add backup search UI elements for Custom Fields tab
            if name == "Custom Fields":
                # Pass the current frame, not the non-existent 'container'
                self._add_backup_search_ui(frame)
    ...
    def _add_backup_search_ui(self, parent_frame: ttk.Frame) -> None:
        """Adds backup searching UI to the custom fields recover tab."""
        backup_frame = ttk.LabelFrame(parent_frame, text="Search Backups for Custom Field Data")
        backup_frame.pack(padx=10, pady=10, fill="x")

        # Backup Path Input
        path_frame = ttk.Frame(backup_frame)
        path_frame.pack(fill="x", padx=5, pady=5)
        ttk.Label(path_frame, text="Backup Folder Path:").pack(side="left")
        self.backup_path_entry = ttk.Entry(path_frame, width=50)
        self.backup_path_entry.pack(side="left", padx=5, fill="x", expand=True)
        ttk.Button(path_frame, text="Browse", command=self._browse_backup_path).pack(side="left")

        # Field Names Input
        names_frame = ttk.Frame(backup_frame)
        names_frame.pack(fill="x", padx=5, pady=5)
        ttk.Label(names_frame, text="Field Names (comma-separated):").pack(side="left")
        self.field_names_entry = ttk.Entry(names_frame, width=50)
        self.field_names_entry.pack(side="left", padx=5, fill="x", expand=True)
        ttk.Button(names_frame, text="Scan Backups", command=self._scan_backups).pack(side="left")
    ...
    def get_deleted_fields(self) -> List[Dict[str, Any]]:
        ...
        Some tenants do not support `field/search?status=deleted`, so we avoid failing hard and
        optionally merge from our JSON backups when the UI toggle is enabled.
        """
        api_rows: List[Dict[str, Any]] = []

        # 1) Prefer Jira API with status=deleted (supported on many Cloud tenants). Ignore failures.
        try:
            ...
            pass  # _get already surfaced an error

        # 2) Optionally merge from backups based on the UI checkbox
        backup_rows: List[Dict[str, Any]] = []
        if self.recover_include_backups.get():
            backup_rows = self._list_from_backups("Custom Fields")

        # 3) Merge uniquely, preferring human-friendly names
        return self._merge_unique(api_rows, backup_rows, id_key="id", item_type="Custom Fields")
    ...
    def recover_deleted_fields(self, field_ids: List[str]) -> List[str]:
        """Recreate deleted custom fields from backups.
        Jira Cloud has no public REST to *restore from trash*, so we re-create
        the field using its backed-up definition (name, type, searcherKey).
        - Searches both full backups and per-item backups.
        - If a name conflict occurs, retries once with " (restored)" suffix.
        - Respects global Dry Run.
        Note: Contexts/scope are not recreated here; add them manually if needed.
        """
        recreated: List[str] = []
        if not field_ids:
            return recreated
        ...
        def iter_candidate_bundles():
            """Yield tuples of (path, top_level_json) for all JSON backups."""
            for path in self._iter_backup_json_paths():
                try:
                    with open(path, "r", encoding="utf-8") as f:
                        data = json.load(f)
        ...
        def extract_field_objects(obj: Any) -> List[Dict[str, Any]]:
            """Return a list of Jira field objects from a backup JSON of any supported shape."""
            out: List[Dict[str, Any]] = []
            if not isinstance(obj, dict):
                return out

            # Per-item field backup: the top-level object *is* the field
            if any(k in obj for k in ("schema", "custom", "searcherKey")) and obj.get("name"):
                out.append(obj)
                return out

            # Full backup format: { itemType: "Custom Fields", items: { id: { field: {...} } } }
            items = obj.get("items") if isinstance(obj.get("items"), dict) else None
            if items and (obj.get("itemType") in (None, "Custom Fields")):
                for _id, bundle in items.items():
                    fld = (bundle or {}).get("field") or {}
                    if fld:
                        out.append(fld)
            return out

        def find_backup_for(target: str) -> Optional[Dict[str, Any]]:
            """Find a field object by ID first, then by name (case-insensitive)."""
            target_lower = str(target).strip().lower()
            by_name_candidates: List[Dict[str, Any]] = []
            for path, data in iter_candidate_bundles():
                for fld in extract_field_objects(data):
                    fid = str(fld.get("id") or "").strip()
                    fname = str(fld.get("name") or "").strip()
                    if fid and fid.lower() == target_lower:
                        return fld
                    if fname and fname.lower() == target_lower:
                        by_name_candidates.append(fld)
            # fallback: any name matches collected (prefer the one with a schema.custom key)
            if by_name_candidates:
                by_name_candidates.sort(key=lambda f: 1 if (f.get("schema") or {}).get("custom") else 0, reverse=True)
                return by_name_candidates[0]
            return None

        def build_create_payload(fld: Dict[str, Any], name_override: Optional[str] = None) -> Dict[str, Any]:
            schema = (fld.get("schema") or {})
            type_key = schema.get("custom") or fld.get("type")  # prefer schema.custom
            body: Dict[str, Any] = {
                "name": name_override or fld.get("name") or "Recovered Field",
                "type": type_key or "com.atlassian.jira.plugin.system.customfieldtypes:textfield",
            }
            # Include searcherKey when available to align behavior with the original field
            if fld.get("searcherKey"):
                body["searcherKey"] = fld.get("searcherKey")
            if fld.get("description"):
                body["description"] = fld.get("description")
            return body

        for fid in field_ids:
            # Locate the backed-up definition
            fld_def = find_backup_for(fid)
            if not fld_def:
                log_to_file("recover", f"No backup found for custom field '{fid}'.")
                continue

            # Dry-run mode: report and skip network calls
            if self.dry_run.get():
                name_preview = str(fld_def.get("name") or fid)
                log_to_file("recover", f"[DRY RUN] Would recreate custom field '{name_preview}'.")
                recreated.append(name_preview)
                continue

            # Try to create the field
            base_url = f"{self.current_domain}/rest/api/3/field"
            name_original = str(fld_def.get("name") or fid)

            def try_create(with_name: str) -> Tuple[bool, Optional[str]]:
                payload = build_create_payload(fld_def, name_override=with_name)
                try:
                    r = requests.post(base_url, auth=self.jira_auth(), json=payload, timeout=TIMEOUT)
                    if r.status_code in (200, 201):
                        new_id = (r.json() or {}).get("id") if r.headers.get("content-type", "").startswith("application/json") else None
                        log_to_file("recover", f"Recreated field '{with_name}' (new id: {new_id or 'unknown'}).")
                        return True, with_name
                    # Name conflict → 400/409 often with message about duplicate name
                    if r.status_code in (400, 409) and "name" in (r.text or "").lower():
                        log_to_file("recover", f"Name conflict for '{with_name}', will retry with suffix. Response: {r.status_code} {r.text[:200]}")
                        return False, None
                    log_to_file("recover", f"Create failed for '{with_name}': {r.status_code} {r.text[:300]}")
                    return False, None
                except Exception as e:
                    log_to_file("recover", f"Create exception for '{with_name}': {e}")
                    return False, None

            ok, used_name = try_create(name_original)
            if not ok:
                # Retry with a safe suffix
                alt_name = f"{name_original} (restored)"
                ok, used_name = try_create(alt_name)

            if ok and used_name:
                recreated.append(used_name)
            else:
                log_to_file("recover", f"Failed to recreate custom field '{name_original}'.")
    ...
    def recover_deleted_filters(self, filter_ids: List[str]) -> List[str]:
        ...
        for old_id in filter_ids:
            ...
            name = filt.get("name")# path: main_updated.py
# Jira Cleanup & Recovery Tool – status bar (thread-safe), checkboxes, Excel export
import re  # add this
import json
import os
...
class JiraToolGUI(tk.Tk):
    ...
    def _list_from_backups(self, item_type: str) -> List[Dict[str, Any]]:
        ...
        # DEEP SCAN: walk everything under BACKUP_DIR
        if self.recover_deep_scan.get():
            for path in self._iter_backup_json_paths():
                try:
                    with open(path, "r", encoding="utf-8") as f:
        ...
        # SHALLOW: try latest full backup first
        data = self._load_full_backup(item_type)
        add_many(data)

        # FALLBACK: if still empty, scan recent per-item backups in the typed subfolder
        if not rows:
            try:
                subdir = os.path.join(BACKUP_DIR, item_type.replace(" ", "_").lower())
                if os.path.isdir(subdir):
                    candidates = [os.path.join(subdir, fn) for fn in os.listdir(subdir)
                                  if fn.lower().endswith(".json")]
                    candidates.sort(key=os.path.getmtime, reverse=True)
                    for path in candidates[:100]:  # cap to keep it snappy
                        try:
                            with open(path, "r", encoding="utf-8") as f:
                                add_many(json.load(f))
                        except Exception as e:
                            log_to_file("recover", f"Skip unreadable per-item backup {path}: {e}")
            except Exception as e:
                log_to_file("recover", f"Per-item backup scan failed: {e}")
