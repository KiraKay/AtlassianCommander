...
class JiraToolGUI(tk.Tk):
    ...
    def recover_deleted_fields(self, field_ids: List[str]) -> List[str]:
        ...
        # Process requests for fields that can be restored from the Cloud trash
        for fid in field_ids:
            if self.dry_run.get():
                log_to_file("recover", f"[DRY RUN] Would recover custom field {fid} from Atlassian Cloud trash")
                recovered.append(fid)
                continue

            # Atlassian provides a 60-day trash period for deleted custom fields
            # A PUT request to restore the field is not available in the public API.
            # Instead, you must use the user interface by navigating to Custom fields > Trashed.
            # Or, if recently deleted, an issue's changelog may have the last known value.

            # NOTE: Your original method's _post(url) call attempts to re-create the field,
            # which will not restore the data if it exists in trash. The Atlassian REST API
            # does not have an endpoint to restore a field from trash.
            # This part of your code needs to be handled manually or by using changelog data.

            # Example for handling a trashed field (requires manual GUI action)
            log_to_file("recover",
                        f"Manual recovery needed for custom field {fid}. Check the 'Trashed' tab in Jira settings.")
