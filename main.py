...
class JiraToolGUI(tk.Tk):
    ...
    def delete_custom_field(self, field_id: str) -> bool:
        """
        Delete a custom field in Jira Cloud using the supported flow:
        - Lookup via /rest/api/3/field/search?id=<id>
        - Backup minimal field payload (name/id/etc.)
        - Move to trash: POST /rest/api/3/field/{id}/trash
        - Fallback: hard delete via DELETE /rest/api/3/field/{id} (303 expected)
        """
        if self.dry_run.get():
            log_to_file("cleanup", f"[DRY RUN] Would delete custom field: {field_id}")
            return True

        # --- 1) Fetch minimal details for backup (supported in v3) ---
        field_data = {}
        try:
            ...
            # Save under backup/backups/custom_fields/ with timestamp handled by backup_to_file()
            backup_to_file(
        ...
        # --- 2) Preferred delete flow: move to trash ---
        ...
        # --- 3) Fallback: hard delete (Jira returns 303 See Other; treat as success) ---
        try:
    ...
    def delete_workflow(self, workflow_name: str) -> bool:
        ...
        # Construct data for backup
        workflow_data = {"name": workflow_name, "id": workflow_name}
        safe_name = re.sub(r"[^A-Za-z0-9_-]+", "_", workflow_name).strip("_") or "unnamed"

        backup_to_file(
            workflow_data,
            f"{safe_name}_Workflows",
            item_type="Workflows"
        )

        # (Delete step removed in Cloud)
        self._show_message(
            "info",
            "Not Supported by Jira Cloud API",
            "Deleting workflows via REST is not available in Jira Cloud.\n"
            "You can remove a workflow from schemes, but full deletion must be done in Jira Admin → Issues → Workflows.",
        )
        log_to_file("cleanup", f"Workflow delete requested but not supported: {workflow_name}")
        return False
    ...
    def get_deleted_fields(self) -> List[Dict[str, Any]]:
        ...
        # Try both status values some tenants expose
        for status_value in ("trashed", "deleted"):
            try:
                ...
                # _get already surfaced an error; keep going
                pass
    ...
    def recover_deleted_fields(self, field_ids: List[str]) -> List[str]:
        ...
        Requirements for success:
          - The field must currently be *trashed* (soft-deleted), not hard-deleted.
          - Caller must have Admin permissions.
        Returns a list of IDs that were successfully restored.
        """
        recovered: List[str] = []
    ...
    def recover_deleted_statuses(self, status_ids: List[str]) -> List[str]:
        ...
        for sid in status_ids:
            if self.dry_run.get():
                log_to_file("recover", f"[DRY RUN] Would recover workflow status {sid}")
                recovered.append(sid)
                continue
            url = f"{self.current_domain}/rest/api/3/status/{sid}/restore"
            if self._post(url):
                recovered.append(sid)
                log_to_file("recover", f"Recovered workflow status {sid}")
        return recovered
